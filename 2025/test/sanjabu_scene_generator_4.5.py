...
parser.add_argument("--scene_num", default=0, help="", type=int)
parser.add_argument("--env", default="Logistic_site", help="", type=str)
...
simulation_app = SimulationApp({"headless":False, "limit_cpu_threads": 300})
...
output_path =  "/nas/Dataset/Dataset_2025/test/" + env_conf["name"] 

cam_model_conf_path = "/home/cubox/ochansol/isaac_code/python/sanjabu/2025/configure/percipio_FM855-E1_conf.json"
...
while scene_num<1000:
    ...
    for model_attr in model_list:
        if model_attr["size_rank"] == size_rank:
            size_sampled_model_list +=[model_attr["name"]]*model_attr["envs"][env_conf["name"]]
    ...
    csr.scatter_in_specipic_area(obj_rep_all_list[0],obj_rep_all_list[1:])
    ...
    if writer.get_data()["annotators"]["instance_segmentation_fast"]["Replicator"]["idToSemantics"].keys().__len__()<7 or \
        writer.get_data()["annotators"]["instance_segmentation_fast"]["Replicator"]["data"][...,-1].min() == 0:

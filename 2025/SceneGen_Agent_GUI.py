...
class MultiPCController:
            ...
            if data.get("type") == "status":
                ...
                if client_name in self.status_vars:
                    if isinstance(scene_num, int):
                        self.status_vars[client_name]['current_scene'].set(f"{scene_num:04d}")
                    else:
                        self.status_vars[client_name]['current_scene'].set(scene_num)
                    
            elif data.get("cmd") == "stop":
                # 클라이언트가 작업 중지를 알려줌
                client_name = data.get("name", client_id)
                print(f"[WORK STOPPED] {client_name} - Stopped")
                ...
                # 클라이언트가 작업 완료를 알려줌
                client_name = data.get("name", client_id)
                print(f"[WORK COMPLETED] {client_name} - All tasks completed")
                
                # GUI에서 상태를 Completed로 변경
                if client_name in self.status_vars:
                    self.status_vars[client_name]['status'].set("Completed")
                    self.status_vars[client_name]['color'].config(bg="blue")  # 완료는 파란색
                    self.status_vars[client_name]['current_scene'].set("Completed")
    ...
    def auto_register_pc(self, client_id):
        ...
        if any(pc['name'] == client_id for pc in self.pc_list):
            # 기존 PC의 연결 상태만 업데이트하고 실행상태/현재scene 초기화
            self.update_connection_status(client_id, True)
            # 재연결 시 실행상태와 현재 Scene 초기화
            if client_id in self.status_vars:
                self.status_vars[client_id]['status'].set("Idle")
                self.status_vars[client_id]['color'].config(bg="gray")
                self.status_vars[client_id]['current_scene'].set("-")
                print(f"[RECONNECTED] {client_id} - Status reset to Idle")
            return
    ...
    def stop(self, pc_info):
        # 클라이언트에게 stop 명령 전송 (상태 변경은 클라이언트 응답 후 처리)
        self.send_command_to_client(pc_info, "stop")

...
4. 원하는 열린 자세에서 **Save START**를 누른 뒤, 닫힌 자세에서 **Save END**를 누릅니다.
5. **Duration**에 전체 전환 시간을 입력하고 **Preview START -> END**로 직선 보간 경로와 접촉을 확인합니다.
...
Base 위치와 orientation, joint는 전체 transition 동안 START에서 END까지 직접 보간합니다. 위치는 선형 보간하고 orientation은 quaternion SLERP를 사용합니다.
...
- Preview는 timeline이 시작된 직후 PhysX articulation을 START joint 값으로 즉시 동기화합니다. 이후 joint와 base TF는 START→END로 직접 진행하므로 joint 0→START 준비 동작은 미리보기에 포함되지 않습니다.

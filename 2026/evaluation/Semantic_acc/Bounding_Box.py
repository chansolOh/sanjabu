...
result_path = os.path.join(file_path, "../result/Semantic_acc/bbox")
...
class bbox_display():
    def __init__(self, root_path, rgb_path, bbox_path, sampling_path_list ):
        ...
        self.img_frame.pack(side=tk.TOP, pady=(0,20))
        ...
        self.window.bind("<space>", lambda x: self.class_name_vis_space() if self.window.focus_get() != self.scene_entry else None)


        ###### init ######
        self.scene_num = -1
        self.bbox_line_width = 2
        self.bbox_color_list = ["#FF0000","#00FF00","#0000FF","#00FFFF","#FF00FF"]
    ...
    def data_load(self, scene_path):
        ...
        with open(os.path.join(self.root_path, middle_path, self.bbox_path, f"{scene_num:04d}.json")) as f:
        ...
        self.plt_class = []
        self.plt_legend = []
        for i,key in enumerate(self.bbox.keys()):
            xmin, ymin, xmax, ymax = self.bbox[key]
            self.plt_bbox+=[
                self.ax.plot(
                    [xmin, xmax, xmax, xmin, xmin], 
                    [ymin, ymin, ymax, ymax, ymin], 
                    c=self.bbox_color_list[i], linewidth=self.bbox_line_width, label=key)]
            tmp = self.ax.text(xmin+15, ymin+30, key, fontsize=15, color="black")
            tmp.set_path_effects([path_effects.withStroke(linewidth=3, foreground=self.bbox_color_list[i])])
            self.plt_class+=[tmp]
            self.plt_legend+=[self.ax.legend(loc="upper right")]
    ...
    def slider_move(self, val):
        if self.plt_bbox is not []:
            ...
            for bbox in self.plt_bbox:
                bbox[0].set_linewidth(self.bbox_line_width)
            self.can_plt.draw()
...
bbox_display(root_path, rgb_path, bbox_path, sampling_path_list)
...
with open(os.path.join(file_path,"syntatic_config.json"), "r") as f:
...
for env in env_list:
    ...
    for section in section_list:
        ...
        for platform in platform_list:
            ...
            total_data_num += len(sample_list)
            # bar_w = 32

            for i, scene_num in enumerate(tqdm(sample_list)):
                with open(os.path.join(root_path,   env,section,platform, inst_seg_path, f"semantics_mapping_{scene_num:04d}.json"),'r', encoding='utf-8') as f:
                    inst_seg_label = json.load(f)
                with open(os.path.join(root_path,   env,section,platform, bbox_path, f"{scene_num:04d}.json"),'r', encoding="utf-8") as f:
                    bbox_label = json.load(f)
                with open(os.path.join(root_path,   env,section,platform, grasp_path, f"{scene_num:04d}.json"),'r', encoding="utf-8") as f:
                    grasp_label = json.load(f)
                with open(os.path.join(root_path,   env,section,platform, scene_meta_path, f"{scene_num:04d}.json"),'r', encoding="utf-8") as f:
                    scene_meta_label = json.load(f)
                
                grasp_data_num += len(grasp_label)
...
inst_seg_acc_total = (total_data_num - inst_seg_err_cnt) / total_data_num *100
bbox_acc_total     = (total_data_num - bbox_err_cnt) / total_data_num *100
grasp_acc_total    = (grasp_data_num - grasp_err_cnt) / grasp_data_num *100
scene_meta_acc_total = (total_data_num - scene_meta_err_cnt) / total_data_num *100


print("==============================================")
print(f"inst_seg err : {inst_seg_err_cnt} / {total_data_num} incorrect")
print(f"bbox err : {bbox_err_cnt} / {total_data_num} incorrect")
print(f"grasp err : {grasp_err_cnt} / {grasp_data_num} incorrect")
print(f"scene_meta err : {scene_meta_err_cnt} / {total_data_num} incorrect")
print("==============================================")

print("inst_seg_acc_total : ",    inst_seg_acc_total, "%")
print("bbox_acc_total : ",        bbox_acc_total, "%")
print("grasp_acc_total : ",       grasp_acc_total, "%")
print("scene_meta_acc_total : ",  scene_meta_acc_total, "%")

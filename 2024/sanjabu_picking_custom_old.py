...
import scan_rep
import Robot_Task_custom_schunk_parallel as Robot_Task
...
parser.add_argument('--scene_num', type=int, default =96 ,help='directory')
...
robot_task = Robot_Task.RobotTask(
    name="robot_task",
    usd_prim_path="/World/Custom_schunk_parallel",)
...
# scene_num = 0
root_path = "/media/nia/SSD1/ochansol_pc2/isaac/sanjabu/Dataset_2024/scene_bak_20241128/Home/"
...
for obj in obj_rep_list:
    ...
    gripper_width_list = [0.15]#[0.15,0.11,0.05,]#0.15,0.11,0.05
    for gripper_width in gripper_width_list:
        ...
        for rot_deg in range(180,0,-30):
            ...
            for bbox_num in range(len(gripper_bbox_3d)):
                ...
                margin = 0.005 # default = 0.005
                if crit<0.008:
                    continue
                    
                if crit>robot_task.gripper_depth/3*2:
                    target_z = palm_max_depth + margin - robot_task.gripper_depth/3*3
                else:
                ...
                if grasp_success_dict["success"]:
                    ...
                    org_grasp_bbox = np.array([[  robot_task.gripper_height/2, -grasp_success_dict["width"]/2 -grasp_success_dict["center"] -  robot_task.finger_thickness, 0],
                                                [-robot_task.gripper_height/2, -grasp_success_dict["width"]/2 -grasp_success_dict["center"] -  robot_task.finger_thickness, 0],
                                                [-robot_task.gripper_height/2,  grasp_success_dict["width"]/2 -grasp_success_dict["center"] +  robot_task.finger_thickness, 0],
                                                [ robot_task.gripper_height/2,  grasp_success_dict["width"]/2 -grasp_success_dict["center"] +  robot_task.finger_thickness, 0] ]).T

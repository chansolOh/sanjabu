...
class RobotTask(tasks.BaseTask):
    ...
    def set_up_scene(self, scene: Scene) -> None:
        ...
        self.gripper_height = 0.018
        self.gripper_depth = 0.148
        self.finger_thickness = 0.028
        self.plot = []
    ...
    def set_robot(self) -> SingleManipulator:
        self.gripper_model = "Custom_schunk_parallel"
        # asset_path = f"omniverse://192.168.0.27/ochansol/sanjabu/Robot/{self.gripper_model}/{self.gripper_model}.usd"
        
        asset_path = f"/media/nia/SSD1/ochansol_pc2/isaac/sanjabu/Robot/{self.gripper_model}/{self.gripper_model}.usd"

        finger_physics_mat_path = f"/World/{self.gripper_model}/Body/schunk_parallel/PhysicsMaterial"
        add_reference_to_stage(usd_path=asset_path, prim_path=self.usd_prim_path)
        gripper = ParallelGripper(
            end_effector_prim_path=f"/World/{self.gripper_model}/Body/schunk_parallel/base",
    ...
    def set_solver(self):
        kinematics_solver = LulaKinematicsSolver(
                            ...
                            # robot_description_path = f"omniverse://192.168.0.27/ochansol/sanjabu/Robot/{self.gripper_model}/{self.gripper_model}_description.yaml",
                            # robot_description_path = "omniverse://192.168.0.27/ochansol/sanjabu/Robot/Custom_schunk_parallel/Custom_schunk_parallel_description.yaml",

                            robot_description_path=f"/media/nia/SSD1/ochansol_pc2/isaac/sanjabu/Robot/{self.gripper_model}/{self.gripper_model}_description.yaml",
                            urdf_path=f"/media/nia/SSD1/ochansol_pc2/isaac/sanjabu/Robot/{self.gripper_model}/{self.gripper_model}.urdf",
        )
    ...
    def picking(self, target_points, target_orientation, target_width, target_prim_path):    #### exact offset J6 : 0.118
        ...
        target_position[2] = 1 - (target_position[2] + self.gripper_depth)
        self.set_init_pose([target_position[0],target_position[1],0.0], [0,0,target_orientation[2]])
        print("robot_position", target_position)
        ...
        if self.grasp_state: 
            ...
            return_dict["center"] = (self._robot.get_joints_state().positions[1] + self._robot.get_joints_state().positions[2])/2
    ...
    def set_contact_sensor(self):
        ...
        self.gripper_L_path = f"/World/{self.gripper_model}/Body/schunk_parallel/clamp_L"
        self.gripper_R_path = f"/World/{self.gripper_model}/Body/schunk_parallel/clamp_R"

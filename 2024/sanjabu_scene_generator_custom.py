...
env_conf = {
    "name": "Home",
    # "usd_path": "/media/nia/6d737125-0a20-46a5-94bc-b44a6aec1a2e/ochansol/isaac_sim/sanjabu/Home/Home_scene1_2204_flatten.usd",
    "usd_path": "/media/nia/Data/peel3/scan_data_2204/cubox_env.usd",
    "position":[0, 0, 0],
    "orientation":[90,0,0],
    "scale":[0.01,0.01,0.01]
}
output_path = "/home/nia/ochansol/isaac/sanjabu/data/noise2/" + env_conf["name"]

cam_model_conf_path = "/home/nia/ochansol/isaac/sanjabu/configure/azure_kinect_conf.json"
...
house_usd = add_reference_to_stage(usd_path=env_conf["usd_path"], 
                                    prim_path="/World/"+env_conf["name"])

house_prim = Prims.XFormPrim(name =env_conf["name"], prim_path="/World/"+env_conf["name"], 
...
light_list = csr.find_lights(house_usd)
...
# model_list = [i for i in obj_attr if "cubox" in i["owner"]]
# model_usd_list = np.array(sorted([i["path"] for i in model_list]))
model_usd_list = ["/media/nia/Data/peel3/scan_data_2204/headphone_box/edited/headphone_box.usd",
              "/media/nia/Data/peel3/scan_data_2204/kitchen_towel/edited/kitchen_towel.usd",
              "/media/nia/Data/peel3/scan_data_2204/cleansing_foam2/edited/cleansing_foam2.usd",
              "/media/nia/Data/peel3/scan_data_2204/stevia/edited/stevia.usd"]
...
while scene_num<20:
    ...
    obj_rep_list = [plane_object]
    active_idx = np.random.choice(len(obj_rep_all_list), 1, replace=False)
    ...
    for objrep in obj_rep_list[1:]:
        csr.set_random_tf(objrep.prim, center_position = [0,0,0.22], scale = [0,0,0])
        # objrep.set_pose(position = [0,0,0.22], rotation = [0,0,0])
    my_world.reset()
    my_world.stop()
    my_world.play()
    # for objrep in obj_rep_list[1:]:
    #     # csr.set_random_tf(objrep.prim, center_position = [0,0,0.22], scale = [0,0,0],rotation=0)
    #     objrep.set_pose(position = [0,0,0.22], rotation = [0,0,0])
    ...
    scene_num+=1
    # while True:
    #     data_list = [i for i in os.listdir(os.path.join(output_path,"distance_to_camera")) if len(i.split("."))>1]
    ...
    # depth_img_new = (np.cos(np.arctan(np.sqrt((dx-cx)**2+(dy-cy)**2)/( (fx+fy)/2 )  ))*depth_img.flatten()).reshape((cam_conf["output_size"][1],cam_conf["output_size"][0]))
    # np.save( depth_img_path.split("distance")[0]+"depth"+depth_img_path.split("distance")[-1], depth_img_new)
    

    # [shutil.move( os.path.join(output_path , i) , os.path.join(output_path,"rgb",scene_name+".png"  ) ) for i in data_list if i.split("_")[0] == "rgb"]
    # [shutil.move( os.path.join(output_path , i) , os.path.join(output_path,"distance_to_camera", scene_name+".npy") ) for i in data_list if i.split("_")[0] == "distance"]
    
    # [shutil.move( os.path.join(output_path , i) , os.path.join(output_path,"depth", scene_name+".npy") ) for i in data_list if i.split("_")[0] == "depth"]
    # bbox_list =  [i for i in data_list if i.split("_")[0] == "bounding"]
    # seg_list = [i for i in data_list if i.split("_")[0] == "instance"]
    # [shutil.move( os.path.join(output_path , i) , os.path.join(output_path,"bbox_2d", scene_name+".npy") ) for i in bbox_list if i.split(".")[-1] == "npy"]
    # [shutil.move( os.path.join(output_path , i) , os.path.join(output_path,"bbox_2d", scene_name+".json") ) for i in bbox_list if i.split(".")[-1] == "json"]
    
    # [shutil.move( os.path.join(output_path , i) , os.path.join(output_path,"inst_seg", scene_name+".png") ) for i in seg_list if i.split(".")[-1] == "png"]
    # [shutil.move( os.path.join(output_path , i) , os.path.join(output_path,"inst_seg", scene_name+".json") ) for i in seg_list if i.split(".")[-1] == "json"]

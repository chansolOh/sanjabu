...
dir_path = "/media/nia/Data/dataset/ycb_video/data/0005/"
mat_list = sorted([os.path.join(dir_path,i) for i in os.listdir(dir_path) if i.split(".")[-1] in ["mat"]])[:50]
for frame in mat_list:
    ...
    for key in ycb_usd_prim_dict:
        ycb_usd_prim_dict[key]["xform_prim"].set_world_pose(position = np.array([0,0,0]),
                                orientation = rot_utils.euler_angles_to_quat((0,0,0),degrees = True))   
        ...
        cache = bounds_utils.create_bbox_cache()
        rot_mat = ycb_tf[:3,:3]
        obb = bounds_utils.compute_obb_corners(cache, ycb_usd_prim_dict[key]["xform_prim"].prim_path )
        center = np.mean(obb,axis=0)
        
        ycb_obb = rot_mat.dot((obb).T).T +ycb_position

        ycb_usd_prim_dict[key]["xform_prim"].set_world_pose(position = ycb_position,
                                orientation = ycb_quat)
        ycb_usd_prim_dict[key]["TF"] = ycb_tf
        ycb_usd_prim_dict[key]["position"] = ycb_position
        ycb_usd_prim_dict[key]["quat"] = ycb_quat
        ycb_usd_prim_dict[key]["obb"] = ycb_obb
